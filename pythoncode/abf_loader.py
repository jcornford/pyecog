...
def get_fs(f):
    abfreader = Abf(f)
    d = abfreader.abfload()
    us_interval  = np.ceil(d[1]['si'])
    fs_khz = int((1000.0/us_interval)[0])
    return fs_khz
...
def get_si(f):
    ...
    d = abfreader.abfload()
    data_dict = d[1]
    si= data_dict['fADCSampleInterval']
    return si
...
if __name__ == '__main__':
    ...
    fs_dictionary = {}
    count = 0
    basedir0 = '/Volumes/LACIE SHARE/VM_data/All_Data_Jan_2016/PV_Arch/'
    basedir1 = '/Volumes/LACIE SHARE/VM_data/All_Data_Jan_2016/PV_ChR2/'
    basedir2 = '/Volumes/LACIE SHARE/VM_data/All_Data_Jan_2016/Som_ChR2/'
    for basedir in [basedir0,basedir1,basedir2]:
        print '*********', basedir, '*********'
    ...
    c2 = 0
    for key in fs_dictionary.keys():
        c2 += 1
    print c2,'keys in dict'

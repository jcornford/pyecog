...
def extract_features(dataset):
    ...
    dataset_bs = dataset - mean_baseline_vector[:,None]

    masked_std_above_threshold = np.ma.masked_where(rolling_std_array < threshold, dataset_bs)
    baseline_length = np.array()
    event_dataset = [np.ma.compressed(masked_std_above_threshold[i,:]) for i in range(dataset.shape[0])]

    print("DONE base subtract")
...
for i in masked_std_below_threshold[6,:200]:
    print i

...
### Recommended installation procedure from pip:
...
### Recommended procedure for running PyECoG from Github source:
1. Follow steps 1 and 2 from above
2. Download the latest source code from [here](https://github.com/jcornford/pyecog/archive/master.zip) and extract.
3. Navigate to the folder and open a terminal/cmd there (or cd across to it).
```{bash}
source activate pyecog_env  or  activate pyecog on windows
python
>>> import pyecog
>>> pyecog
<module 'pyecog' from 'YOUR-PATH_HERE/pyecog/pyecog/__init__.py'>
```
